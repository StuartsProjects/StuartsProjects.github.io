...
I chose an Arducam OV2640 as the camera together with the Arducam library. This software was easy to use and does the job very well. I like this camera, it uses hardware SPI and I2C for the interface, so if the host Arduino is already using these interfaces then only one extra IO pin is needed, the SPI CS pin for the camera. 

The Arducam library, link here; [**Arducam Library**](https://github.com/ArduCAM/Arduino), provides the routines for setting up and using the camera and there is a provided example, **ArduCAM\_Mini\_Capture2SD** that takes pictures and saves them to an SD card. 
