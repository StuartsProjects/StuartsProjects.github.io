...
#### LoRa® and LoRa®WAN are different

LoRa® devices are used for simple point to point communications. LoRa®WAN is an extra software based network communication layer added to basic LoRa® that allows sensors or nodes to communicate with gateway devices in a structured manner. These gateways can forward data received from nodes across the Internet to applications and data collection services running on remote servers. Internet services such as Cayenne allow you to view data from sensors on-line.

The Things Network (TTN) is a community based free to use network of LoRa®WAN gateways. If you want to have a sensor monitor temperatures, water levels, or track the location of assets, you can use the TTN to do this, assuming a gateway is within range.
